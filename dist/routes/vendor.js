 ...
 */
router.get('/servicios_base', async (req, res) => {
    try {
...
router.post('/mis_servicios', async (req, res) => {
    try {
        ...
        if (vendor.plan.limite_servicios !== null) {
            const count = await prisma_1.default.miServicio.count({
 ...
 */
router.delete('/mis_servicios/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const servicio = await prisma_1.default.miServicio.findFirst({
            where: { id, vendor_id: req.vendor.id },
        ...
        await prisma_1.default.miServicio.update({
            where: { id: id },
            data: { activo: false },
        });
 ...
 */
router.get('/imagen/:id/download', async (req, res) => {
    try {
        const vendor = req.vendor;
        const { id } = req.params;
        const imagen = await prisma_1.default.imagen.findFirst({
        ...
        const processedImage = await (0, watermark_1.applyWatermark)(imagen.url_base, watermarkText, logoUrl, whatsapp);
 ...
 */
router.post('/pagos/comprobante', cloudinary_1.upload.single('comprobante'), async (req, res) => {
    try {
        const vendor = req.vendor;
        ...
        const final_comprobante_url = file ? (0, cloudinary_1.getFileUrl)(file) : (comprobante_url || null);
        ...
        try {
            onesignal_1.OneSignal.sendNotification({
 ...
 */
router.get('/ajustes-publicos', async (_req, res) => {
    try {
 ...
 */
router.post('/marketplace/propose', async (req, res) => {
    try {
        ...
        const proposal = await prisma_1.default.servicioBase.create({
            data: {
 ...
 */
router.get('/public/u/:alias', async (req, res) => {
    try {
 ...
 */
router.get('/mis_servicios/historial', async (req, res) => {
    try {
        const vendor_id = req.vendor?.id;
        const historial = await prisma_1.default.miServicio.findMany({
            where: { vendor_id },
            include: {
                servicio: {
